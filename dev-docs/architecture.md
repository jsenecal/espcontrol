...
| Native panel document | `components/espcontrol/panel_config_document.h` and `src/webserver/model/panel_config.ts` | Bounded, versioned `PanelConfig` binary codec shared by firmware and the browser. V1 carries current compact card strings unchanged; live storage and API migration follow in later phases. |
